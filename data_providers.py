...
class RSNADataProvider(DataProvider):
    """Data provider for RSNA kaggle challenge images"""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, flatten=False, mode="undefined"):
        """Create a new MNIST data provider object.

        Args:
            which_set: One of 'train', 'valid' or 'test'. Determines which
                portion of the RSNA data this object should provide.
            batch_size (int): Number of data points to include in each batch.
            max_num_batches (int): Maximum number of batches to iterate over
                in an epoch. If `max_num_batches * batch_size > num_data` then
                only as many batches as the data can be split into will be
            ...
            shuffle_order (bool): Whether to randomly permute the order of
                the data before each epoch.
            rng (RandomState): A seeded random number generator.
            mode (str): defines which kinds of targets are used
                '2 classes': classify into no opacity (0) - opacities (1)
                '3 classes': classify into normal (0) - no opacity/not normal (1) - opacities (2)
                'segmentation': targets for segmentation (currently list of list 
                                                          with bounding boxes, waiting for Francisco's input)
        """
        # check a valid which_set was provided
        assert which_set in ['train', 'valid', 'test'], (
        ...
        assert mode in ['2 classes', '3 classes', 'segmentation'], "Please enter valid mode"
        assert mode in ['2 classes', '3 classes'], "Segmentation not implemented yet, because not yet clear how the targets should look like"
        
        self.mode = mode
        if self.mode == '2 classes': 
            self.num_classes = 2
        elif self.mode == '3 classes':
            self.num_classes = 3
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            "data", 'RSNA_{0}.npz'.format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
        loaded = np.load(data_path)
        if self.mode == '2 classes': 
            inputs, targets = loaded['inputs'], loaded['targets2']
        elif self.mode == '3 classes':
            inputs, targets = loaded['inputs'], loaded['targets3']
        else:
            inputs, targets = loaded['inputs'], loaded['targets_seg']
        
        inputs = inputs.astype(np.float32) #!! That is something that we might want to change...
        
        if flatten:
            inputs = np.reshape(inputs, newshape=(-1, 1024*1024))
        inputs = inputs / 255.0
        
        # pass the loaded data to the parent class __init__
        super(RSNADataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(RSNADataProvider, self).next()
        return inputs_batch, self.to_one_of_k(targets_batch)
...
# =============================================================================
    ...
    def __init__(self, which_set, mode=None,
                 transform=False, gamma_factor=1, rot_angle=0, shear_angle=0, translate_distance=0, scale_factor=1):
        ...
        assert mode in ['2 classes', '3 classes', 'segmentation', "multitask with 2 classes"], "Please enter valid mode"
        ...
        if self.mode == '2 classes' or self.mode == "multitask with 2 classes": 
            self.num_classes = 2
        elif self.mode == '3 classes':
            self.num_classes = 3
        else: #!! currently undefined
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            self.data_path_base, "RSNA_PyTorch_Provider_{0}.npz".format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        ...
        if self.mode == '2 classes' or self.mode == 'segmentation' or self.mode == "multitask with 2 classes": 
    ...
    def __getitem__(self, index):
        ...
        """
        mode_with_segmentation = self.mode == "segmentation" or self.mode == "multitask with 2 classes"
        ...
        # load target, depending on mode
        target_class = self.targets[index]
        target_class = target_class.astype(np.int64)

        if mode_with_segmentation:
        ...
        if self.mode == "multitask with 2 classes":
            return image, (target_class, target_image)
